...
void GameMenuManager::UpdateSettings()
{
    ...
    {
        D3DXCreateFontA(TheRenderManager->device, textSize, 0, FW_NORMAL, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                        ANTIALIASED_QUALITY, FF_DONTCARE, MenuSettings.TextFont, &TheGameMenuManager->FontNormal);
        D3DXCreateFontA(TheRenderManager->device, textSize, 0, FW_BOLD, 1, false, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                        ANTIALIASED_QUALITY, FF_DONTCARE, MenuSettings.TextFont, &TheGameMenuManager->FontSelected);
        D3DXCreateFontA(TheRenderManager->device, MenuSettings.TextSizeStatus * ratio, 0, FW_NORMAL, 1, false,
                        DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_DONTCARE,
                        MenuSettings.TextFontStatus, &TheGameMenuManager->FontStatus);
    ...
    TheGameMenuManager->pageSize = (TheRenderManager->height - (TheGameMenuManager->MenuHeight + TheGameMenuManager->
        margins)) / (TheGameMenuManager->rowHeight);
...
void GameMenuManager::MainMenuMessage()
{
    ...
    std::string menuMessage = std::string(PluginVersion::VersionString) + " - Open the Config Menu by pressing the key "
        + GetKeyName(MenuSettings.KeyEnable);

    SetRect(&Rect, 0, TheRenderManager->height - textSize - 10, TheRenderManager->width,
            TheRenderManager->height + textSize);
...
void GameMenuManager::HandleInput()
{
...
#ifdef debugMenuInput
    Logger::Log("Selected column: %i, selected row: %i, Selected page: %i", SelectedColumn, SelectedRow[SelectedColumn],
                SelectedPage[SelectedColumn]);
    ...
    else
    {
        ...
        {
            if (isShaderSection && (SelectedColumn == COLUMNS::CATEGORY || (SelectedColumn == COLUMNS::SETTINGS &&
                isStatusSection)))
            {
                // enable shaders and effects (DX9 or Vulkan)
                const char* shaderName = SelectedNode.MidSection;

                // 1) Vulkan effect?
                bool handled = false;
                if (TheVulkanEffectsManager)
                {
                    if (IVulkanEffect* vulkanEffect = TheVulkanEffectsManager->FindEffectById(shaderName))
                    {
                        vulkanEffect->SetEnabled(true); // updates config via SetMenuShaderEnabled
                        handled = true;
                    }
                }

                // 2) Legacy DX9 shader if not Vulkan
                if (!handled)
                {
            ...
            {
                TESWeather* Weather = (TESWeather*)DataHandler->GetFormByName(
                    SelectedNode.MidSection, TESForm::FormType::kFormType_Weather);
            ...
            TheSettingManager->LoadSettings(); // update SettingsMain etc.
        }
        else if (IsKeyPressed(MenuSettings.KeySubtract))
        {
            if (isShaderSection && (SelectedColumn == COLUMNS::CATEGORY || (SelectedColumn == COLUMNS::SETTINGS &&
                isStatusSection)))
            {
                // disable shaders and effects (DX9 or Vulkan)
                const char* shaderName = SelectedNode.MidSection;
                ...
                if (TheVulkanEffectsManager)
                {
                    if (IVulkanEffect* vulkanEffect = TheVulkanEffectsManager->FindEffectById(shaderName))
                    {
                        // respect "forced" logic if you want, or always allow
                        if (!TheSettingManager->IsShaderForced(shaderName))
                        {
                            vulkanEffect->SetEnabled(false); // updates config
                        }
                        handled = true;
                    }
                }
                ...
                {
                    bool shaderEnabled = TheSettingManager->GetMenuShaderEnabled(shaderName);
                    if (shaderEnabled && !TheSettingManager->IsShaderForced(shaderName))
                    {
                        TheShaderManager->SwitchShaderStatus(shaderName);
                    }
                }
...
void GameMenuManager::Render()
{
    ...
    if (TheSettingManager->hasUnsavedChanges)
        DrawShadowedText(
            std::string(
                "/!\\ You have unsaved changes. To avoid losing them, save them using the " +
                GetKeyName(MenuSettings.KeySave) + " Key").c_str(), MainColumnWidth * 3, 0, MainColumnWidth * 2,
            TextColorEditing, FontNormal, DT_LEFT);
    ...
    for (UInt32 i = 0; i < ListSize; i++)
    {
        ...
        DrawShadowedText(Sections[i].c_str(), MainColumnWidth * i, HeaderYPos, MainColumnWidth, textColor, Font,
                         DT_LEFT);
    }
    ...
    for (UInt32 i = pageSize * SelectedPage[CurrentColumn]; i < min(
             ListSize, pageSize * (SelectedPage[CurrentColumn] + 1)); i++)
    {
        ID3DXFont* Font = FontNormal;
        D3DXCOLOR textColor = TextColorNormal;
        ...
        if (isShaderSection) {
            const char* shaderName = Sections[i].c_str();

            bool enabled = TheSettingManager->GetMenuShaderEnabled(shaderName);
            if (enabled) textColor = TextColorEnabled;

            DrawShadowedText(enabled ? "ENABLED" : "DISABLED",
                             pos + 1, lineYPos, 100, textColor, FontStatus, DT_LEFT);

            if (TheSettingManager->SettingsMain.Develop.DebugMode) {
                float total = 0.0f;

                // 1) DX9 effect?
                if (EffectRecord* effect = TheShaderManager->GetEffectByName(shaderName)) {
                    total = max(effect->renderTime + effect->constantUpdateTime, 0.0f);

                    // Shadows special case
                ...
                // 2) Vulkan effect?
                else if (TheVulkanEffectsManager) {
                    total = TheVulkanEffectsManager->GetEffectGpuTimeMs(shaderName);
                }

                if (!TheSettingManager->SettingsMain.Main.RenderEffects)
                    total = 0.0f;
                ...
                DrawShadowedText(duration.c_str(),
                                 0, lineYPos, ItemColumnWidth - textSize,
                                 TextColorNormal, FontNormal, DT_RIGHT);
            }
        }
    }
             ...
             ListSize, pageSize * (SelectedPage[CurrentColumn] + 1)); i++)
    {
        ...
        DrawShadowedText(Sections[i].c_str(), ItemColumnWidth * (CurrentColumn - 1),
                         MenuHeight + rowHeight * (i % pageSize), ItemColumnWidth, textColor, Font, DT_LEFT);
    }
             ...
             ListSize, pageSize * (SelectedPage[CurrentColumn] + 1)); i++)
    {
        ...
        DrawShadowedText(SettingText, ItemColumnWidth * (CurrentColumn - 1), MenuHeight + rowHeight * (i % pageSize),
                         ItemColumnWidth, textColor, FontNormal, DT_LEFT);
        Setting++;
    }
    ...
    // render description
    DrawShadowedText(DescriptionText, ItemColumnWidth, HeaderYPos, ItemColumnWidth * 2, TextColorNormal, FontNormal,
                     DT_LEFT);
    ...
    std::string KeysInfo = "Page " + std::to_string(SelectedPage[COLUMNS::CATEGORY] + 1) + "/" + std::to_string(
            Pages[COLUMNS::CATEGORY] + 1) +
        " | Keybinds: Enable/Increment: " + GetKeyName(MenuSettings.KeyAdd) + ", Disable/Decrement: " + GetKeyName(
            MenuSettings.KeySubtract) +
        ", Start Editing value: " + GetKeyName(toggleEntry) + ", Save Settings: " + GetKeyName(MenuSettings.KeySave);

    DrawShadowedText(KeysInfo.c_str(), 0, HeaderYPos + (pageSize + 1) * rowHeight + RowSpace * 4 + LineThickness,
                     MainColumnWidth * 2, TextColorNormal, FontNormal, DT_LEFT);
