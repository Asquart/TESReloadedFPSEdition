...
Before dispatching compute work the Vulkan layer transitions the color and depth images to layouts that are appropriate for compute access:

* Color: `COLOR_ATTACHMENT_OPTIMAL → GENERAL`
...
* Normals: the vanilla export does not include a precomputed normal buffer. To expose one, allocate a render target on the Direct3D side, register it with `RegisterTextureInterop`, and publish it via `TextureManager::PublishBridgeState`. After updating the Vulkan layer to bind it, declare another `sampler2D` or `image2D` binding and sample it normally.
* Shadow maps or other game buffers: follow the same export flow. The bridge supports up to four handles per surface, so you can forward both Direct3D pointers and Vulkan objects. On the Vulkan side make sure the declared descriptor type matches the usage flags you set when registering the texture (e.g., `TR_BRIDGE_RT_USAGE_SAMPLED_BIT` → sampled image, `TR_BRIDGE_RT_USAGE_STORAGE_BIT` → storage image).
